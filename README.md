...
**v1.17** is the current release — makes the Phase 4 reverse-lint actually run (it was dead on plugin-scope installs and scanned zero files everywhere else, while reporting "clean"), adds a bundled `resolve_dep.sh` for resolving sibling-plugin scripts at both install roots, and extends the skill-freshness audit to plugin-scope skills. **v1.13** added the review-findings audit (handoff §7 table + step 24d merge into the usage record), hardened subagent-usage verification (including the unit-mismatch variant: a non-zero scalar can still be ~2× off the itemized truth), a new-model rate-table caveat, next-prompt gating (write one only when a next action is recommended), and docs-only handoff-PR auto-merge — on top of the v1.9 bucket-aware dispatch.
